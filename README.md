...
> **Live:** _paste your deployed URL here_
...
them — but most run on a paper ledger and a shared spreadsheet, which means
...
shelf space** — the last one being what decides the next grant application.
...
| `/desk` | keeper | Check tools back in, add donations, retire tools — the Saturday-morning screen |
...
```
React 19 + Vite ── React Router 7 ── Zustand 4 stores
        │  fetch, access token in memory, refresh cookie
        ▼
Express 4  ── requireAuth → requireRole → validate → route
        ▼
SQLite (better-sqlite3, WAL) — users · tools · loans
```
...
- A **partial unique index** — `UNIQUE(tool_id) WHERE returned_on IS NULL` —
...
Access token in **memory only** — never `localStorage`, which any XSS can read.
...
| | member | keeper |
|---|---|---|
| browse the catalogue | ✔ (no account needed) | ✔ |
| borrow | ✔ 3 tools, 7 days | ✔ 6 tools, 14 days |
| extend own loan | ✔ | ✔ |
| see others' loans | ✘ | ✔ |
| check tools in | ✘ | ✔ |
| add / retire tools | ✘ | ✔ |
...
1. **Keeper dashboard** — four headline figures each with the previous period
...
2. **Dark mode** — follows the OS until you override it. Only the palette is
...
3. **CI/CD and Docker** — GitHub Actions runs API tests → client tests → bundle
   budget → Lighthouse. Multi-stage Dockerfile, non-root, healthchecked. Render
...
cd server
cp .env.example .env
node -e "console.log(require('crypto').randomBytes(48).toString('hex'))"   # ×2 into .env
npm install
...
filesystem is ephemeral — SQLite in the working directory loses every account on
...
- **Postgres.** SQLite has been the right call — one file, no service, real
  transactions — but it pins the API to one instance. The data layer is behind
...
Three bugs in this codebase were found by tests, not by looking at it — all in
...
1. **An infinite render loop.** Zustand v5 compares selector output with
   ...
   `useShallow` fixed it.
2. **The API envelope unwrapped twice** — `const { data } = await api('/tools')`
...
Every one of those would have shipped. It also changed how I write mocks — the
...
- [ ] Push to GitHub, deploy both halves, paste the live URL at the top
- [ ] Record the walkthrough: catalogue → open a tool → sign in as a member →
      borrow it → see it on `/loans` → extend it → sign in as a keeper → check it
      back in at the desk → dashboard → **toggle dark mode** → show it on a phone
